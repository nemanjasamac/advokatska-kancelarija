...
@section('title', 'Zakaži termin - Advokatska Kancelarija')

@section('content')
<div class="mb-6">
    <h1 class="text-3xl font-bold text-gray-800">Zakaži termin</h1>
    <p class="text-gray-600">Zakazivanje sastanka ili ročišta</p>
</div>
...
<div class="bg-white rounded-lg shadow p-6 max-w-2xl">
    <form action="{{ route('appointments.store') }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <div class="flex space-x-4">
                <label class="flex items-center">
                    <input type="radio" name="type" value="sastanak" {{ old('type', 'sastanak') === 'sastanak' ? 'checked' : '' }} class="mr-2">
                    Sastanak
                </label>
                <label class="flex items-center">
                    <input type="radio" name="type" value="rociste" {{ old('type') === 'rociste' ? 'checked' : '' }} class="mr-2">
                    Ročište
                </label>
            </div>
        </div>
        ...
        <div class="mb-4">
            ...
            <input type="text" name="location" id="location" value="{{ old('location') }}" placeholder="npr. Osnovni sud Beograd, Kancelarija..."
                class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500">
        </div>
        ...
        <div class="flex justify-end space-x-3">
            <a href="{{ route('appointments.index') }}" class="px-4 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50">
                Odustani
            </a>
