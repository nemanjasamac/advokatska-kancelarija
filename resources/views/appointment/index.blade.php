...
<div class="flex justify-between items-center mb-6">
    ...
    <a href="{{ route('appointments.create') }}" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg flex items-center">
                ...
                <tr class="hover:bg-gray-50 {{ $appointment->date_time < now() ? 'opacity-50' : '' }}">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                        @if($appointment->legalCase)
                            <a href="{{ route('legal-cases.show', $appointment->legalCase) }}" class="text-blue-600 hover:underline">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm">
                        <a href="{{ route('appointments.edit', $appointment) }}" class="text-indigo-600 hover:text-indigo-900 mr-3">Izmeni</a>
                        <form action="{{ route('appointments.destroy', $appointment) }}" method="POST" class="inline" onsubmit="return confirm('Da li ste sigurni?')">
                            @csrf
                            @method('DELETE')
