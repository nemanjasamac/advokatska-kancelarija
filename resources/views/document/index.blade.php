...
<div class="flex justify-between items-center mb-6">
    ...
    <a href="{{ route('documents.create') }}" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg flex items-center">
        ...
        <tbody class="bg-white divide-y divide-gray-200">
            @forelse($documents as $document)
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                        <a href="{{ route('legal-cases.show', $document->legalCase) }}" class="text-blue-600 hover:underline">
                            {{ $document->legalCase->title }}
                        </a>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-gray-600">{{ $document->uploaded_at->format('d.m.Y.') }}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm">
                        <form action="{{ route('documents.destroy', $document) }}" method="POST" class="inline" onsubmit="return confirm('Da li ste sigurni?')">
                            @csrf
                            @method('DELETE')
