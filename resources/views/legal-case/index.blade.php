...
<div class="flex justify-between items-center mb-6">
    ...
    <a href="{{ route('legal-cases.create') }}" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg flex items-center">
            ...
            @forelse($legalCases as $case)
                <tr class="hover:bg-gray-50">
                    <td class="px-6 py-4 whitespace-nowrap">
                        <a href="{{ route('legal-cases.show', $case) }}" class="text-blue-600 hover:underline font-medium">
                            {{ $case->title }}
                        </a>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
                        <a href="{{ route('clients.show', $case->client) }}" class="text-gray-600 hover:text-blue-600">
                            {{ $case->client->name }}
                        </a>
                    </td>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm">
                        <a href="{{ route('legal-cases.edit', $case) }}" class="text-indigo-600 hover:text-indigo-900 mr-3">Izmeni</a>
                        <form action="{{ route('legal-cases.destroy', $case) }}" method="POST" class="inline" onsubmit="return confirm('Da li ste sigurni?')">
                            @csrf
                            @method('DELETE')
